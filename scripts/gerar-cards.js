...
// Gera cards-YYYY-MM-DD.html com dados em tempo real da API da Caixa.
...
function contarAcumulados(historicoPath, ultimoConcursoAPI) {
  try {
    const h = JSON.parse(require('fs').readFileSync(historicoPath, 'utf8'));
    const draws = h.draws;
    if (!draws) return 0;
    ...
    for (let i = keys.length - 1; i >= 0; i--) {
      ...
      if (p && typeof p === 'object') {
        // Mega-Sena: gs = ganhadores sena (6/6)
        // Quina:     g5 = ganhadores quina (5/5)
        // Lotofácil: g15 = ganhadores 15/15
        ganhadores = p.gs ?? p.g5 ?? p.g15 ?? 0;
      }
      if (ganhadores === 0) count++;
      else break;
    }
    // Se a API confirma que o último concurso também acumulou, adiciona 1
...
// ─── TEMPLATE HTML ───────────────────────────────────────────────────────────
function gerarHTML(mega, facil, quina, today) {
  const todayStr   = ptbrDate(today);
  const fileDate   = today.toISOString().slice(0,10);

  function badgeText(d, nome) {
  ...
  // Mega
  const megaBadge = badgeText(mega, 'Mega-Sena');
  const megaTitle = mega.acumulado
  ...
  // Lotofácil
  const facilBadge = badgeText(facil, 'Lotofácil');
  const facilTitle = `Lotofácil<br><em>sorteio hoje.</em>`;

  // Quina
  const quinaBadge = badgeText(quina, 'Quina');
  const quinaTitle = quina.acumulado
...
.card-mega .brand-mark { color: rgba(244,235,208,0.2); }
...
.card-mega .top { position: relative; z-index: 1; }
.card-mega .badge {
...
.card-mega .draw-date { color: #f4ebd0; }

/* ═══ CARD 2 — LOTOFÁCIL ════════════════════════════════════════════════════ */
.card-facil { background: #07030F; }
.card-facil .brand-mark { color: rgba(244,235,208,0.18); }
...
.card-facil .top { position: relative; z-index: 1; }
.card-facil .badge {
  background: rgba(123,47,190,0.2);
  border: 1px solid rgba(123,47,190,0.5);
  color: #b06ef0;
}
.card-facil .badge-dot {
  background: #b06ef0;
  box-shadow: 0 0 8px rgba(176,110,240,0.8);
}
.card-facil .headline {
  font-size: 72px;
  font-weight: 800;
...
.card-facil .cta-btn {
  background: #7B2FBE;
  color: #f4ebd0;
  box-shadow: 0 0 32px rgba(123,47,190,0.4);
}
.card-facil .draw-date { color: #f4ebd0; }

/* ═══ CARD 3 — QUINA ════════════════════════════════════════════════════════ */
.card-quina { background: #02080F; }
.card-quina .brand-mark { color: rgba(244,235,208,0.18); }
...
.card-quina .top { position: relative; z-index: 1; }
.card-quina .badge {
  background: rgba(107,140,218,0.14);
  border: 1px solid rgba(107,140,218,0.4);
  color: #6b8cda;
}
.card-quina .badge-dot {
  background: #6b8cda;
  box-shadow: 0 0 8px rgba(107,140,218,0.8);
}
.card-quina .headline {
  font-size: 68px;
  font-weight: 800;
...
.card-quina .cta-btn {
  background: rgba(107,140,218,0.18);
  border: 1.5px solid rgba(107,140,218,0.5);
  color: #6b8cda;
}
.card-quina .draw-date { color: #f4ebd0; }

/* ═══ CARD 4 — ANALISADOR ═══════════════════════════════════════════════════ */
.card-analisa { background: #060D0A; }
.card-analisa .brand-mark { color: rgba(244,235,208,0.18); }
...
.card-analisa .top { position: relative; z-index: 1; }
.card-analisa .badge {
  background: rgba(212,168,75,0.1);
  border: 1px solid rgba(212,168,75,0.35);
  color: #d4a84b;
}
.card-analisa .badge-dot {
  background: #d4a84b;
  box-shadow: 0 0 8px rgba(212,168,75,0.6);
}
.card-analisa .headline {
  font-size: 72px;
  font-weight: 800;
  line-height: 1.0;
  letter-spacing: -0.03em;
...
.num-ball.cold {
  background: rgba(107,140,218,0.12);
  border: 2px solid rgba(107,140,218,0.35);
  color: #6b8cda;
}
.num-ball.mid {
  background: rgba(244,235,208,0.06);
  border: 2px solid rgba(244,235,208,0.15);
...
.bar-track {
  height: 6px;
  background: rgba(244,235,208,0.07);
  border-radius: 3px;
  overflow: hidden;
}
...
<div class="guide">
  ...
  Gerado automaticamente em ${new Date().toLocaleTimeString('pt-BR')} · Dados em tempo real da API da Caixa.<br><br>
...
<p class="card-label">Card 1 · Mega-Sena · Concurso ${mega.proximo}</p>
<div class="card card-mega">
  ...
  <div class="top">
    ...
    <div class="headline">${megaTitle}</div>
  </div>

  <div class="mega-prize-block">
    <div class="prize-label">Prêmio estimado · Concurso ${mega.proximo} · ${mega.dataProx || todayStr}</div>
    <div class="prize-value"><span class="rs">R$</span>&nbsp;${formatPremio(mega.premio)}</div>
    <div class="prize-sub">palpitiar.com.br/mega-sena</div>
  </div>

  <div class="bottom">
    <div class="cta-btn">Gerar combinações grátis →</div>
  </div>
  <div class="mascot-bubble">
    <img src="./coruja.png" alt="Coruja Palpitiar">
  </div>
</div>

<!-- ══════════════════════════════════════════════════════════════════════════ -->

<p class="card-label">Card 2 · Lotofácil · Concurso ${facil.proximo}</p>
<div class="card card-facil">
  ...
  <div class="top">
    <div class="badge">
      <span class="badge-dot"></span>
      Lotofácil · ${facilBadge}
    </div>
    <div class="headline">${facilTitle}</div>
  ...
  <div class="facil-prize-block">
    <div class="prize-label">Prêmio estimado · Concurso ${facil.proximo} · ${facil.dataProx || todayStr}</div>
    <div class="prize-value"><span class="rs">R$</span>&nbsp;${formatPremio(facil.premio)}</div>
    <div class="prize-sub">palpitiar.com.br/lotofacil</div>
  </div>

  <div class="bottom">
    <div class="cta-btn">Gerar dezenas agora →</div>
  </div>
  <div class="mascot-bubble">
    <img src="./coruja.png" alt="Coruja Palpitiar">
  </div>
...
<p class="card-label">Card 3 · Quina · Concurso ${quina.proximo}</p>
<div class="card card-quina">
  ...
  <div class="top">
    ...
    <div class="headline">${quinaTitle}</div>
  </div>

  <div class="quina-prize-block">
    <div class="prize-label">Prêmio estimado · Concurso ${quina.proximo} · ${quina.dataProx || todayStr}</div>
    <div class="prize-value"><span class="rs">R$</span>&nbsp;${formatPremio(quina.premio)}</div>
...
<p class="card-label">Card 4 · Analisador · Teste seus números</p>
<div class="card card-analisa">
  ...
  <div class="top">
    <div class="badge">
      <span class="badge-dot"></span>
      Palpitiar · Analisador
    </div>
    <div class="headline">
      Seus números<br>
      <em>são bons<br>de verdade?</em>
    </div>
  </div>
  ...
  <div class="bottom">
    <div class="cta-btn">Testar meus números →</div>
  </div>
  <div class="mascot-bubble">
    <img src="./coruja.png" alt="Coruja Palpitiar">
  </div>
...
async function main() {
  ...
  const [megaRaw, facilRaw, quinaRaw] = await Promise.all([
    fetchCaixa('megasena'),
    fetchCaixa('lotofacil'),
    fetchCaixa('quina'),
  ]);

  const mega  = extrairDados(megaRaw,  'Mega-Sena', 'mega-sena-historico.json');
  const facil = extrairDados(facilRaw, 'Lotofácil', 'lotofacil-historico.json');
  const quina = extrairDados(quinaRaw, 'Quina',     'quina-historico.json');

  // Relatório
  const linhas = [mega, facil, quina].map(d => {
    if (!d.ok) return `  ❌ ${d.nome} — erro ao buscar`;
    const acc = d.acumulado ? `Acumulada há ${d.acumHa} concurso(s)` : 'Não acumulada';
    return `  ✅ ${d.nome.padEnd(12)} #${d.proximo}  ${formatPremio(d.premio).padStart(14)}  ${acc}  ${d.dataProx || '?'}`;
  });
  console.log(linhas.join('\n') + '\n');

  const today    = new Date();
  const fileDate = today.toISOString().slice(0,10);
  const filename = path.join(process.cwd(), `cards-${fileDate}.html`);

  const html = gerarHTML(mega, facil, quina, today);
  fs.writeFileSync(filename, html, 'utf8');

  console.log(`✅ Arquivo gerado: cards-${fileDate}.html`);
  console.log(`\n📸 Como capturar:`);
  console.log(`   1. Abra o arquivo no Chrome`);
  console.log(`   2. F12 → clique com botão direito em .card → "Capture node screenshot"`);
  console.log(`   3. Repita para cada card (4 no total)`);
