'use strict';
// scripts/preencher-lacunas-diadesorte.js
// Lê o diadesorte-historico.json já existente, identifica quais concursos estão
// faltando dentro do intervalo [primeiro..ultimo], e busca SÓ esses — bem mais
// devagar e sequencial — para não disparar o bloqueio 403 do WAF (Azion).
// Uso: node scripts/preencher-lacunas-diadesorte.js
...
const ARQUIVO    = path.join(process.cwd(), 'diadesorte-historico.json');
const DEBUG_FILE = path.join(process.cwd(), 'debug-diadesorte-lacunas.log');
const DELAY_MS   = 2000;
const CONCURSO_MINIMO = 1; // Dia de Sorte começou em maio/2018 — força varredura desde o concurso 1
  ...
  return new Promise((resolve) => {
    ...
    const req = https.get(url, {
      ...
      headers: {
        ...
        'Referer': 'https://loterias.caixa.gov.br/Paginas/Dia-de-Sorte.aspx',
...
async function main() {
  console.log('=== Preencher lacunas — Dia de Sorte ===');
  ...
  const primeiro = Math.min(CONCURSO_MINIMO, ...json.draws.map(d => d[0]));
