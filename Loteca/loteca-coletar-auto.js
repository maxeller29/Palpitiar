/**
 * loteca-coletar-auto.js
 * Coleta automática da Loteca para GitHub Actions
 * 
 * 1. Busca último concurso na API Caixa
 * 2. Verifica se já existe no banco
 * 3. Se novo: coleta + analisa
 * 4. Saída via GITHUB_OUTPUT para o workflow decidir se notifica
 *
 * Variáveis de ambiente:
 *   SUPABASE_KEY       = sb_secret_...
 *   APIFOOTBALL_KEY    = (opcional)
 */

'use strict';

const https = require('https');
const fs    = require('fs');
const path  = require('path');
const { execSync } = require('child_process');

const SUPABASE_URL    = process.env.SUPABASE_URL || 'https://oslvqimllizsdtxwkrag.supabase.co';
const SUPABASE_KEY    = process.env.SUPABASE_KEY;
const CAIXA_BASE      = 'https://servicebus2.caixa.gov.br/portaldeloterias/api/loteca';
const GITHUB_OUTPUT   = process.env.GITHUB_OUTPUT || null;

if (!SUPABASE_KEY) { console.error('SUPABASE_KEY nao definida'); process.exit(1); }

// ─── Helpers ──────────────────────────────────────────────────────────────────

function httpGet(url, opts) {
  return new Promise((resolve, reject) => {
    const req = https.get(url, { headers: { 'User-Agent': 'Mozilla/5.0' }, ...opts }, (res) => {
      let d = '';
      res.on('data', c => d += c);
      res.on('end', () => {
        if (res.statusCode >= 500) return reject(new Error(`HTTP ${res.statusCode}`));
        if (res.statusCode === 404) return reject(new Error('HTTP 404'));
        try { resolve(JSON.parse(d)); } catch { reject(new Error('nao-JSON')); }
      });
    });
    req.on('error', reject);
    req.setTimeout(12000, () => { req.destroy(); reject(new Error('timeout')); });
  });
}

function sb(method, table, body, params) {
  return new Promise((resolve, reject) => {
    const url = new URL(`${SUPABASE_URL}/rest/v1/${table}`);
    if (params) Object.entries(params).forEach(([k,v]) => url.searchParams.set(k,v));
    const payload = body ? JSON.stringify(body) : null;
    const req = https.request(url, {
      method,
      headers: {
        'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}`,
        'Content-Type': 'application/json', 'Prefer': 'return=representation',
      },
    }, (res) => {
      let d = ''; res.on('data', c => d += c);
      res.on('end', () => {
        if (res.statusCode >= 400) return reject(new Error(`Supabase ${res.statusCode}: ${d.slice(0,200)}`));
        try { resolve(d ? JSON.parse(d) : []); } catch { resolve([]); }
      });
    });
    req.on('error', reject);
    if (payload) req.write(payload);
    req.end();
  });
}

function setOutput(key, value) {
  if (GITHUB_OUTPUT) {
    fs.appendFileSync(GITHUB_OUTPUT, `${key}=${value}\n`);
  }
  console.log(`[output] ${key}=${value}`);
}

function parseDateBR(str) {
  if (!str) return null;
  const [d,m,y] = str.split('/');
  return `${y}-${m}-${d}`;
}

function calcResultado(g1, g2) {
  if (g1 === null || g1 === undefined) return null;
  if (g1 > g2) return 'coluna1';
  if (g1 === g2) return 'empate';
  return 'coluna2';
}

// ─── Main ─────────────────────────────────────────────────────────────────────

(async () => {
  console.log('\n' + '='.repeat(60));
  console.log('LOTECA — COLETA AUTOMATICA');
  console.log('='.repeat(60));

  try {
    // 1. Buscar último concurso publicado na Caixa
    console.log('\n[1/4] Buscando ultimo concurso na API Caixa...');
    let dadosCaixa;
    try {
      dadosCaixa = await httpGet(CAIXA_BASE);
    } catch(e) {
      console.log(`  API Caixa indisponivel: ${e.message}`);
      setOutput('novo_concurso', 'false');
      setOutput('status', 'api_indisponivel');
      process.exit(0);
    }

    const numeroCaixa = dadosCaixa.numero;
    const dataSorteio = parseDateBR(dadosCaixa.dataApuracao);
    console.log(`  Ultimo concurso Caixa: ${numeroCaixa} (${dadosCaixa.dataApuracao})`);

    // 2. Verificar se já existe no banco
    console.log('\n[2/4] Verificando banco de dados...');
    const existentes = await sb('GET', 'loteca_concursos', null, {
      numero: `eq.${numeroCaixa}`, select: 'id,numero,status_analise',
    });

    if (existentes.length > 0) {
      console.log(`  Concurso ${numeroCaixa} ja existe no banco (status: ${existentes[0].status_analise})`);
      setOutput('novo_concurso', 'false');
      setOutput('numero', String(numeroCaixa));
      setOutput('status', existentes[0].status_analise);
      process.exit(0);
    }

    // 3. Novo concurso! Verificar se tem jogos
    const jogosApi = dadosCaixa.listaResultadoEquipeEsportiva || [];
    if (jogosApi.length === 0) {
      console.log(`  Concurso ${numeroCaixa} ainda sem jogos publicados.`);
      setOutput('novo_concurso', 'false');
      setOutput('status', 'sem_jogos');
      process.exit(0);
    }

    console.log(`  NOVO CONCURSO DETECTADO: ${numeroCaixa} com ${jogosApi.length} jogos!`);

    // 4. Inserir concurso
    console.log('\n[3/4] Inserindo concurso e jogos...');

    // Upsert competição
    const slug = 'copa-do-mundo-fifa-2026';
    let compId;
    const compEx = await sb('GET', 'competicoes_config', null, { slug: `eq.${slug}`, select: 'id' });
    if (compEx.length > 0) {
      compId = compEx[0].id;
    } else {
      const compNovo = await sb('POST', 'competicoes_config', {
        slug, label: 'Copa do Mundo FIFA 2026', nome_caixa_pattern: 'Copa do Mundo',
        tipo: 'grupos', nivel: 'copa_mundial', reducao_score_eliminatorio: 0, peso_forma: 1.0,
      });
      compId = compNovo[0].id;
    }

    // Inserir concurso
    const rateioPremio = dadosCaixa.listaRateioPremio || [];
    const faixa14 = rateioPremio.find(r => r.descricaoFaixa?.includes('14'));
    const faixa13 = rateioPremio.find(r => r.descricaoFaixa?.includes('13'));

    const concursoNovo = await sb('POST', 'loteca_concursos', {
      numero: numeroCaixa,
      data_sorteio: dataSorteio,
      status_analise: 'coletando',
      acumulado: dadosCaixa.acumulado || false,
      estimativa_premio: dadosCaixa.valorEstimadoProximoConcurso || null,
      arrecadacao: dadosCaixa.valorArrecadado || null,
      ganhadores_14: faixa14?.numeroDeGanhadores ?? null,
      rateio_14: faixa14?.valorPremio ?? null,
      ganhadores_13: faixa13?.numeroDeGanhadores ?? null,
      rateio_13: faixa13?.valorPremio ?? null,
    });
    const concursoId = concursoNovo[0].id;
    console.log(`  Concurso criado: id=${concursoId}`);

    // Inserir jogos
    const jogosPorOrdem = [...jogosApi].sort((a,b) => a.nuJogo - b.nuJogo);
    const timeCache = {};

    async function upsertTime(nome) {
      if (timeCache[nome]) return timeCache[nome];
      const ex = await sb('GET', 'times_mapeamento', null, { nome_caixa: `eq.${nome}`, select: 'id' });
      if (ex.length > 0) { timeCache[nome] = ex[0].id; return ex[0].id; }
      const novo = await sb('POST', 'times_mapeamento', {
        nome_caixa: nome, nome_popular: nome, pais: 'Internacional',
        nivel_nacional: 'media', fonte_dados: 'manual', ativo: true,
      });
      timeCache[nome] = novo[0].id;
      return novo[0].id;
    }

    for (let idx = 0; idx < jogosPorOrdem.length; idx++) {
      const j = jogosPorOrdem[idx];
      const ordem = idx + 1;
      const timeCasaId  = await upsertTime(j.nomeEquipeUm);
      const timeVisitId = await upsertTime(j.nomeEquipeDois);
      const resultadoReal = calcResultado(j.nuGolEquipeUm, j.nuGolEquipeDois);

      await sb('POST', 'loteca_jogos_analise', {
        concurso_id: concursoId, ordem,
        time_casa_id: timeCasaId, time_visit_id: timeVisitId,
        competicao_id: compId, competicao_label: dadosCaixa.nomeCampeonato || 'Copa do Mundo FIFA 2026',
        is_classico: false, is_eliminatorio: false,
        classificacao: 'dificil', resultado_sugerido: 'coluna1', cobertura: 'triplo',
        resultado_real: resultadoReal, acertou: null, revisado_editor: false,
        resumo: `${j.nomeEquipeUm} x ${j.nomeEquipeDois}`,
        justificativa: JSON.stringify({ nomeEquipeUm: j.nomeEquipeUm, nomeEquipeDois: j.nomeEquipeDois,
          golUm: j.nuGolEquipeUm ?? null, golDois: j.nuGolEquipeDois ?? null, fonte: 'automatico' }),
      });
      console.log(`  [${String(ordem).padStart(2)}] ${j.nomeEquipeUm} x ${j.nomeEquipeDois}`);
    }

    // Atualizar status para rascunho
    await sb('PATCH', 'loteca_concursos', { status_analise: 'rascunho' }, { id: `eq.${concursoId}` });

    console.log('\n[4/4] Concurso coletado. Analise sera executada em seguida.');

    setOutput('novo_concurso', 'true');
    setOutput('numero', String(numeroCaixa));
    setOutput('data_sorteio', dataSorteio || '');
    setOutput('jogos', String(jogosPorOrdem.length));
    setOutput('status', 'coletado');

    console.log('\n' + '='.repeat(60));
    console.log(`NOVO CONCURSO ${numeroCaixa} COLETADO COM SUCESSO`);
    console.log('='.repeat(60));

  } catch(err) {
    console.error(`\nERRO: ${err.message}`);
    setOutput('novo_concurso', 'false');
    setOutput('status', 'erro');
    setOutput('erro_msg', err.message.replace(/\n/g,' '));
    process.exit(1);
  }
})();
