...
(async () => {
  const antes = await supabase('GET', 'loteca_concursos', null, {
    numero: 'eq.1256', select: 'id,estimativa_premio',
  });
  console.log('Valor atual:', antes[0]?.estimativa_premio);

  const res = await supabase('PATCH', 'loteca_concursos',
    { estimativa_premio: 2000000 },
    { numero: 'eq.1256' }
  );
  console.log('Atualizado para: 2000000 (R$ 2.000.000,00)');
})();
